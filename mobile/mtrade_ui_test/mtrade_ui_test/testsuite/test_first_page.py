...
class TestFirstPage:
    ...
    @allure.feature('首页')
    @allure.story('发微淘')
    def test_release_taobao_page(self, first):
        """
        1.点击首页发微淘图标
        2.检查页面跳转是否成功
        :param first:
        :return:
        """
        result = first.go_release_taobao_page()
        first.close()
        assert result, '跳转发微淘页面失败'

    @allure.feature('首页')
    @allure.story('淘大直播')
    def test_taobao_broadcast_page(self, first):
        """
          1.点击首页淘大直播图标
          2.检查页面跳转是否成功
          :param first:
          :return:
          """
        result = first.go_taobao_broadcast_page()
        first.back()
        assert result, '跳转淘大直播页面失败'

    @allure.feature('首页')
    @allure.story('违规词检测')
    def test_violation_words_check_page(self, first):
        """
          1.点击首页违规词检测图标
          2.检查页面跳转是否成功
          :param first:
          :return:
          """
        result = first.go_violation_words_check_page()
        first.refresh()
        first.close()
    ...
    def test_batch_rate(self, go_to_firstbatch_evaluation_page):
        """
        ...
        为避免消耗数据，没有点击立即评价按钮
        """
        info_rate_page = go_to_firstbatch_evaluation_page
