...
class TestBatchSendGoods:
    ...
    def test_go_batch_send_goods_page(self, batch_send_goods_page):
        """
        1.点击首页批量发货按钮，进入批量发货列表
        2.点击列表全部发货按钮，跳转批量发货页面
        3.检查页面是否跳转成功
        :param batch_send_goods_page:
        :return:
        """
        result = batch_send_goods_page.go_batch_send_goods_page2()
        assert result, "跳转页面失败"

    @allure.feature('批量发货')
    @allure.story('选择自己联系发货')
    def test_choose_offline_delivery(self, batch_send_goods_page):
        """
        1.批量发货列表进入批量发货页面
        2.选择自己联系发货方式
        3.检查发货方式是否选择成功
        ...
        :return:
        """
        batch_send_goods_page.go_batch_send_goods_page2()
        result = batch_send_goods_page.choose_offline_delivery()
        assert result, "选择自己联系发货失败"

    @allure.feature('批量发货')
    @allure.story('选择在线下单发货')
    def test_choose_online_delivery(self, batch_send_goods_page):
        """
         1.批量发货列表进入批量发货页面
         2.选择在线下单发货方式
         3.检查发货方式是否选择成功
    ...
    def test_choose_dummy_delivery(self, batch_send_goods_page):
        """
         1.批量发货列表进入批量发货页面
         2.选择无需物流发货方式
         3.检查发货方式是否选择成功
    ...
    @allure.feature('批量发货')
    @allure.story('稍后处理')
    def test_pending_handle(self, batch_send_goods_page):
        """
        ...
        :return:
        """
        batch_send_goods_page.choose_single_order()
        batch_send_goods_page.go_batch_send_goods_page2()
        result = batch_send_goods_page.pending_handle()
        batch_send_goods_page.refresh()
        assert result, "稍后处理失败"

    @allure.feature('批量发货')
    @allure.story('选择快递公司')
    def test_choose_deliver_company(self, batch_send_goods_page):
        """
        1.进入批量发货列表，选择单笔订单，跳转批量发货页面
        2.选择快递公司为顺丰速运
        3.检查快递公司是否选择成功
        ...
        batch_send_goods_page.choose_single_order()
        batch_send_goods_page.go_batch_send_goods_page2()
        result = batch_send_goods_page.choose_deliver_company()
        assert result, "选择快递失败"

    @allure.feature('批量发货')
    @allure.story('全部发货')
    @pytest.mark.skip(reason="避免消耗所有订单数据")
    def test_all_deliver(self, init_app):
        """
        1.进入批量发货列表，选择单笔订单，跳转批量发货页面
        2.点击全部发货
