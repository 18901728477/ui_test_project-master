...
class TestItemList:
    ...
    @allure.story('下架宝贝')
    @pytest.mark.p1
    def test_off_shelf(self, go_onsale_list_scan):
        list_page = go_onsale_list_scan
    ...
    @allure.story('复制链接')
    @pytest.mark.p2
    def test_copy_link(self, go_onsale_list_scan):
        link = go_onsale_list_scan.copy_link_list()
    ...
    @allure.story('查看详情')
    @pytest.mark.p2
    def test_view_detail(self, go_onsale_list_scan):
        list_page = go_onsale_list_scan
        assert list_page.search_by_keyword(ITEM_ONSALE_TITLE, poco(text=ITEM_ONSALE_TITLE)), '宝贝不存在'
        res = go_onsale_list_scan.view_detail_list(ITEM_ONSALE_TITLE)
        assert res, '跳转详情失败'
    ...
    def test_two_dimensional_code(self, go_onsale_list_scan):
        res = go_onsale_list_scan.two_dimensional_code_list()
        assert res, '二维码下载失败'

    @allure.feature('宝贝列表')
    @allure.story('促销推广')
    @pytest.mark.p1
    def test_promotion(self, go_onsale_list_scan):
        res = go_onsale_list_scan.promotion_list()
        assert res, 'ump不能正常跳转、创建活动'

    @allure.feature('宝贝列表')
    @allure.story('标题优化')
    @pytest.mark.p1
    def test_title_optimization(self, go_inventory_list_scan):
        go_inventory_list_scan.search_by_keyword(ITEM_INVENTORY_TITLE, poco(text=ITEM_INVENTORY_TITLE))
        res = go_inventory_list_scan.title_optimization_list()
        assert res, '不能正常一键优化'

    @allure.feature('宝贝列表')
    @allure.story('手机详情')
    @pytest.mark.p1
    def test_mobile_detail(self, go_inventory_list_scan):
        res = go_inventory_list_scan.mobile_detail_list()
        assert res, '手机详情跳转失败'

    @allure.feature('宝贝列表')
    @allure.story('宝贝可以正常批量上下架')
    @pytest.mark.p1
    def test_batch_uplist_downlist(self, go_inventory_list_scan):
        """
        1.进入仓库中列表
        ...
        :return:
        """
        list_page = go_inventory_list_scan
    ...
    @pytest.mark.p1
    def test_activity_calendar(self, go_inventory_list_scan):
        assert go_inventory_list_scan.activity_calendar_shop_test_enter()

    @allure.feature('宝贝列表')
    @allure.story('可以正常批量生成手机详情')
    @pytest.mark.p1
    def test_batch_mdetail(self, go_onsale_list_scan):
        """
        1.进入出售中列表
        2.搜索关键词“保温杯”
        3.点击批量，全选宝贝，点击“手机详情”
