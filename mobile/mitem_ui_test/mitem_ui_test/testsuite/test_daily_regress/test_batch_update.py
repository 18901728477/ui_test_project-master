...
class TestBatchUpdate(BasePage):
    ...
    @allure.story('批量修改价格')
    @pytest.mark.p1
    def test_batch_price(self, first_page_wait):
        batch_update_page = first_page_wait.go_batch_update()
    ...
    @allure.story('批量修改标题')
    @pytest.mark.p2
    def test_batch_title(self, first_page_wait):
        batch_updata_page = first_page_wait.go_batch_update()
    ...
    @allure.story('批量修改库存')
    @pytest.mark.p2
    def test_batch_stock(self, first_page_wait):
        batch_update_page = first_page_wait.go_batch_update()
    ...
    @pytest.mark.p1
    def test_update_desc(self, first_page_wait):
        """
        1.进入批量修改-添加描述tab
        2.添加描述：我添加的描述+时间戳
    ...
    @pytest.mark.p2
    def test_list_delist(self, first_page_wait):
        """
        1.进入批量修改首页，点击批量下架
        ...
        assert list_page.search_by_keyword('批量修改', poco(textMatches='编码:.*')), '宝贝不存在'
        list_page.choose_item(1)
