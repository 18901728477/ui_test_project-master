...
def run_with_report(test_case_dir: str, test_report_dir: str) -> None:
    ...
    :return:
    """
    pytest.main([test_case_dir, '-vv', '--reruns=2', '--reruns-delay=2',
                 '--alluredir={}/xml'.format(test_report_dir)])
    # 常用参数说明:
    ...
    os.system(r'allure generate {}/xml -o {}/html --clean'.format(test_report_dir, test_report_dir))


if __name__ == '__main__':
    root_path = os.path.dirname(os.path.abspath(__file__))  # __file__表示当前文件自身，观察下run.py所在路径，可以知道root_path
    # 表示项目根目录，即test_by_poco/
    test_dir = os.path.join(root_path, 'testsuite')  # 找到测试脚本所在目录,root_path/testsuite
    daily_test = os.path.join(test_dir, 'test_for_developer')
    now_time = datetime.datetime.now()
    time_suffix = now_time.strftime('%Y%m%d_%H%M%S')  # 固定语法获取时间戳
    report_dir = os.path.join(root_path, 'report_' + time_suffix)  # 拼接字符串，使得每次能够能到不同的测试报告，防止测试结果数据冲突
    run_with_report(daily_test, report_dir)  # 根据需要选择测试目录 testsuite为根目录，包括所有测试用例
